 ...
 *
 */
public class TvRecyclerBinder extends BaseRecyclerViewBinder<TvRecyclerViewHolder, TvSeries> {

    private static final int DEFAULT_IMAGE_PLACEHOLDER = R.drawable.ic_movie;

    protected TvRecyclerBinder(final Context context, final IntentDispatcher intentDispatcher) {
        super(context, intentDispatcher);
    }

    @Override
    public void bind(final TvRecyclerViewHolder holder,
                     final TvSeries item) {
        if (item != null) {
            holder.title.setText(item.getName());
            holder.subTitle.setText(item.getFirstAirDate());
        }
    }

    @Override
    public void reset(final TvRecyclerViewHolder holder) {
